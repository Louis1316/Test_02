...
def image_tf_judge(img):
    ...
    inner_mask_S = []
    inner_mask_counter = []
    outer_mask = [mask_blue + mask_yellow + mask_green,
                  mask_red + mask_yellow + mask_green]
    outer_mask_S = []
    outer_mask_counter = []
    features = {"inner_color": "none", "inner_shape": "none",
                "outer_color": "none"}  # 内部颜色 内部形状 外部颜色 外部形状
    sd = ShapeDetector()
    detect = 0
    ...
    if detect != 0:
        x, y, w, h = cv2.boundingRect(outer_mask_counter[outer_tag])
        img_boundingRect = cv2.rectangle(
            img, (x, y), (x + w, y + h), (0, 0, 0), 3)
        cv2.putText(img_boundingRect,
                    features["inner_color"] + "_" +
                    features["inner_shape"] + "_" + features["outer_color"],
                    (int(x + w / 2), int(y + h / 2)),
                    cv2.FONT_HERSHEY_SIMPLEX,
                    0.4, (0, 0, 0), 1)
        cv2.imshow("img", img_boundingRect)
        print(features["inner_color"] + "_" +
              features["inner_shape"] + "_" + features["outer_color"])
